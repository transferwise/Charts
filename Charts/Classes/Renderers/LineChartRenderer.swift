...
public class LineChartRenderer: LineRadarRenderer
{
    ...
    public override func drawData(context context: CGContext)
    {
        ...
        for i in 0 ..< lineData.dataSetCount
        {
            ...
            if set.isVisible
            {
                ...
                drawDataSet(context, dataSet: set as! ILineChartDataSet)
            }
        }
    }
    ...
    public func drawDataSet(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        case .Stepped:
            drawLinear(context, dataSet: dataSet)
            
        case .CubicBezier:
            drawCubicBezier(context, dataSet: dataSet)
            
        case .HorizontalBezier:
            drawHorizontalBezier(context, dataSet: dataSet)
        }
    ...
    public func drawCubicBezier(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        if (dataSet.isDrawFilledEnabled)
        {
            ...
            drawCubicFill(context, dataSet: dataSet, spline: fillPath!, matrix: valueToPixelMatrix, bounds: _xBounds)
        }
        
        if let gradientEndColor = dataSet.gradientEndColors.first where _xBounds.range > 1
        {
            let baseSpace = CGColorSpaceCreateDeviceRGB()
            let gradient = CGGradientCreateWithColorComponents(baseSpace, drawingColor.components() +
                gradientEndColor.components(), nil, 2)
                ...
                bounds: XBounds)
    {
        ...
        if dataSet.fill != nil
        {
            drawFilledPath(context, path: spline, fill: dataSet.fill!, fillAlpha: dataSet.fillAlpha)
        }
        else
        {
            drawFilledPath(context, path: spline, fillColor: dataSet.fillColor, fillAlpha: dataSet.fillAlpha)
        }
    }
    ...
    public func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        {
            drawLinearFill(context, dataSet: dataSet, trans: trans, bounds: _xBounds)
        }
    ...
    public func drawLinearFill(context: CGContext, dataSet: ILineChartDataSet, trans: Transformer, bounds: XBounds)
    {
        guard let dataProvider = dataProvider else { return }
        ...
        if dataSet.fill != nil
        {
            drawFilledPath(context, path: filled, fill: dataSet.fill!, fillAlpha: dataSet.fillAlpha)
        }
        else
        {
            drawFilledPath(context, path: filled, fillColor: dataSet.fillColor, fillAlpha: dataSet.fillAlpha)
        }
    }
        ...
        for i in 0 ..< dataSets.count
        {
            ...
            if dataSet.drawOnlyLastCircle
            {
                drawCircle(context, dataSet: dataSet, j: max(0, Int(ceil(Double(_xBounds.max - _xBounds.min) * phaseX + Double(_xBounds.min))) - 1), phaseY: phaseY,
                           ...
                           drawCircleHole: drawCircleHole, valueToPixelMatrix: valueToPixelMatrix)
            }
            else
            {
                for j in _xBounds.min.stride(to: Int(ceil(Double(_xBounds.max - _xBounds.min) * phaseX + Double(_xBounds.min))), by: 1)
                {
                    if !drawCircle(context, dataSet: dataSet, j: j, phaseY: phaseY,
                                   circleRadius: circleRadius, circleDiameter: circleDiameter,
                                   circleHoleRadius: circleHoleRadius, circleHoleDiameter: circleHoleDiameter,
                                   drawTransparentCircleHole: drawTransparentCircleHole,
                                   drawCircleHole: drawCircleHole, valueToPixelMatrix: valueToPixelMatrix)
    ...
    private func drawCircle(context: CGContext, dataSet: ILineChartDataSet, j: Int, phaseY: Double,
                            circleRadius: CGFloat, circleDiameter: CGFloat, circleHoleRadius: CGFloat, circleHoleDiameter: CGFloat,
                            drawTransparentCircleHole: Bool, drawCircleHole: Bool, valueToPixelMatrix: CGAffineTransform) -> Bool
    {
        var pt = CGPoint()
        var rect = CGRect()
        
        guard let
            e = dataSet.entryForIndex(j),
            let viewPortHandler = self.viewPortHandler
            else { return false }
    ...
    public override func drawHighlighted(context context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            guard let set = lineData.getDataSetByIndex(high.dataSetIndex) as? ILineChartDataSet where set.isHighlightEnabled
                else { continue }
...
extension NSUIColor
{
    func components() -> [CGFloat]
    {
        ...
        let numOfComponents = 4 //device rgb
        
        var array: [CGFloat] = []
        
        for c in 0 ..< numOfComponents
        {
            array.append(components[c])
        }
